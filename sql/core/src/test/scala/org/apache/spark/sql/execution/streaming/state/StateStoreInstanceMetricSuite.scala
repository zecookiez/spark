...
class StateStoreInstanceMetricSuite extends StreamTest with AlsoTestWithRocksDBFeatures {
  ...
  private val ROCKSDB_SNAPSHOT_LAG_METRIC_PREFIX = "rocksdb.SnapshotLastUploaded.partition_"
  private val HDFS_BACKED_SNAPSHOT_LAG_METRIC_PREFIX = "hdfs.SnapshotLastUploaded.partition_"

  private def rocksDBSnapshotLagMetricName(
      partitionId: Long,
      storeName: String = StateStoreId.DEFAULT_STORE_NAME): String = {
    s"$ROCKSDB_SNAPSHOT_LAG_METRIC_PREFIX${partitionId}_$storeName"
  }

  private def hdfsBackedSnapshotLagMetricName(
      partitionId: Long,
      storeName: String = StateStoreId.DEFAULT_STORE_NAME): String = {
    s"$HDFS_BACKED_SNAPSHOT_LAG_METRIC_PREFIX${partitionId}_$storeName"
  }
          ...
          Execute { q =>
            ...
            eventually(timeout(10.seconds)) {
              val instanceMetrics = q.lastProgress
                ...
                .asScala
                .view
                .filterKeys(_.startsWith(ROCKSDB_SNAPSHOT_LAG_METRIC_PREFIX))
            ...
            eventually(timeout(10.seconds)) {
              assert(
                q.lastProgress
                  .stateOperators(0)
                  .customMetrics
                  .get(rocksDBSnapshotLagMetricName(0)) === -1
              )
              assert(
                q.lastProgress
                  .stateOperators(0)
                  .customMetrics
                  .get(rocksDBSnapshotLagMetricName(1)) === -1
              )
              // Make sure only smallest K active metrics are published
              val instanceMetrics = q.lastProgress
                .stateOperators(0)
          ...
          Execute { q =>
            eventually(timeout(10.seconds)) {
              ...
              val badInstanceMetrics = allInstanceMetrics.filterKeys(
                k =>
                  k.startsWith(rocksDBSnapshotLagMetricName(0, "")) ||
                  k.startsWith(rocksDBSnapshotLagMetricName(1, ""))
              )
              // Determined by STATE_STORE_INSTANCE_METRICS_REPORT_LIMIT
              assert(
  ...
  testWithChangelogCheckpointingEnabled(
    "SPARK-51252: Verify snapshot lag metrics are updated correctly with " +
          ...
          Execute { q =>
            ...
            eventually(timeout(10.seconds)) {
              val instanceMetrics = q.lastProgress
                ...
                .asScala
                .view
                .filterKeys(_.startsWith(HDFS_BACKED_SNAPSHOT_LAG_METRIC_PREFIX))
              // Determined by STATE_STORE_INSTANCE_METRICS_REPORT_LIMIT
              assert(
                instanceMetrics.size == q.sparkSession.conf
                  .get(SQLConf.STATE_STORE_INSTANCE_METRICS_REPORT_LIMIT)
              )
              // Because of how HDFSBackedStateStoreProvider uses strict inequality
              // to determine if a snapshot is needed, the first upload is actually
              // done at version 2 instead of 1 (RocksDB uses non-strict inequalities).
            ...
            eventually(timeout(10.seconds)) {
              assert(
                q.lastProgress
                  .stateOperators(0)
                  .customMetrics
                  .get(hdfsBackedSnapshotLagMetricName(0)) === -1
              )
              assert(
                q.lastProgress
                  .stateOperators(0)
                  .customMetrics
                  .get(hdfsBackedSnapshotLagMetricName(1)) === -1
              )
              // Make sure only smallest K active metrics are published
              val instanceMetrics = q.lastProgress
                .stateOperators(0)
              ...
              // Because of how HDFSBackedStateStoreProvider uses strict inequality
              // to determine if a snapshot is needed, the first upload is actually
              // done at version 2 instead of 1.
              assert(
  ...
  testWithChangelogCheckpointingEnabled(
    "SPARK-51252: Verify snapshot lag metrics are updated correctly for join queries with " +
    "HDFSBackedStateStoreProvider"
  ) {
    withSQLConf(
      SQLConf.STATE_STORE_PROVIDER_CLASS.key -> classOf[HDFSBackedStateStoreProvider].getName,
        ...
        testStream(joined)(
          ...
          Execute { q =>
            eventually(timeout(10.seconds)) {
              ...
              // All state store instances should have uploaded a snapshot at version 2
              assert(instanceMetrics.forall(_._2 == 2))
            }
          },
          StopStream
        ...
        testStream(joined)(
          ...
          Execute { q =>
            eventually(timeout(10.seconds)) {
              ...
              val badInstanceMetrics = allInstanceMetrics.filterKeys(
                k =>
                  k.startsWith(hdfsBackedSnapshotLagMetricName(0, "")) ||
                  k.startsWith(hdfsBackedSnapshotLagMetricName(1, ""))
              )
              // Determined by STATE_STORE_INSTANCE_METRICS_REPORT_LIMIT
              assert(
                allInstanceMetrics.size == q.sparkSession.conf
              ...
              // Two ids are blocked, each with four state stores
              assert(badInstanceMetrics.count(_._2 == -1) == 2 * 4)
              // The rest should have uploaded a snapshot at version 2
              assert(
